...
object FromRegexParser extends StringParsers[List] {
  def buildParser(r: Regex): Parser[String] =
    r match
      ...
      case AnyExcept(r)     => not(buildParser(r)).map(_.toString())
      ...
      case NegatedClass(c)  => not(buildParser(c)).map(_.toString())
      ...
      case TimesRange(from, to, r) =>
        repeatedMinN(buildParser(r))(from, to).map(_.mkString)
  ...
  private def parseMoreLzy(m: More): Parser[String] = {
    m match
      ...
      case Times(n, r) => repeatedN(buildParser(r))(n).map(_.mkString)
      case TimesRange(from, to, r) =>
        repeatedMinNLzy(buildParser(r))(from, to).map(_.mkString)
  }
